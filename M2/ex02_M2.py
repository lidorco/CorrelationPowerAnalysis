...
def calculate_key():
    """
    For each key byte (8 time. index i):
        For each key byte value (256 options. index j):

            calculate hypothesis vector: SBOX( HW( data[i] ^  j ) )
            for each trace vector(64 values) generate correlation matrix
